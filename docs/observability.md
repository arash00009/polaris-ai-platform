...
- **Not yet run on the target machine.** Everything in Section 3 is written and statically verified, not executed against a live cluster. Treat every command above as "expected to work", not "confirmed to work", until this document is updated with real output.
