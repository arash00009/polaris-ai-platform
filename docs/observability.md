...
- **Partially run on the target machine, across two real runs (2026-09-24).** The first run got `kube-prometheus-stack` to `STATUS: deployed` (all four pods `Running`) and hit a real Loki chart-usage bug (`deploymentMode`, see `docs/troubleshooting.md`) that blocked it before reaching Loki or Tempo. After that fix (and two others), the second run confirmed `kube-prometheus-stack`, Loki *and* Tempo all install cleanly to `STATUS: deployed`, then hit a fourth real bug at the OTel Collector step — a breaking upstream chart-default change requiring `image.repository` to be set explicitly, also fixed (`docs/troubleshooting.md`). The OTel Collector install itself, and `ai-platform-dev`'s own rollout with observability enabled, are not yet confirmed. Treat those two specifically as "expected to work", not "confirmed to work", until this document is updated again with real output.
