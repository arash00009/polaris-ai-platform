...
Status: accepted. Written; the parts that do not need Docker or a Kubernetes cluster (shell lint, static tests, secret scanning, dependency audit, workflow-file lint) were installed and run for real in the sandbox this project is developed in, with genuine results — see "Consequences". The parts that do need Docker or a cluster (image build/check/scan/SBOM, the ephemeral k3d deploy, the GHCR publish) could not run here (see "Context") and have their first real run on the target machine and, separately, the first time this workflow actually executes on GitHub Actions — this is a new category beyond earlier phases' "sandbox vs. target machine" split, since a GitHub-hosted runner is a third environment neither of those two is.
...
Consequences: real, in the sandbox, on 2026-09-22 — `gitleaks v8.30.0` and `actionlint v1.7.12`'s Linux tarballs were downloaded from GitHub release assets and their sha256 checksums matched the project's own published checksums files before either binary was run; `pip-audit 2.10.1` and `yamllint 1.38.0` were installed from PyPI. `make ci-secrets-scan` found no leaks in the repository; `make ci-deps-audit` found no known vulnerabilities in `app/ai_service/requirements.txt`; `make ci-workflow-lint` (yamllint + actionlint, the latter running shellcheck against every `run:` block) found no issues in `.github/workflows/ci.yml` — but only after actionlint's embedded shellcheck genuinely caught a real bug during development here (SC2046, unquoted word-splitting in the image-save step), fixed before it ever reached GitHub. `make lint` and `make test` (127 static checks, up from 88 at the end of Phase 5) both pass. None of this proves the workflow runs correctly on GitHub Actions itself — Docker was not reachable in the sandbox (`docker info` fails), so `image-build`/`image-check`/`image-scan`/`image-sbom`/the ephemeral k3d cluster/`image-publish` are untested beyond shellcheck and `bash -n`, the same status Phase 4 and Phase 5's scripts had before their first target-machine run. Unlike those phases, though, this workflow's *first* real end-to-end run will not be on the target machine at all — it will be the first time it actually executes on GitHub Actions after a push, which is a claim this project has not had to make before and should not be described as "done" until that run is pasted in here with its actual result, the same "never fabricate a result" rule every earlier phase followed.

Revisit when: the first real GitHub Actions run, successful or not (update this record and `docs/troubleshooting.md` with what actually happened, exactly like every other phase); Phase 7 (the post-deploy verification loop, in the *local* cluster, closes the other half of ADR-12's "two loops"); Phase 8 (a successful `publish` triggers a commit to the separate `polaris-gitops` repository instead of being the last step); Phase 15 (Cosign keyless signing joins the `publish` job; Sealed Secrets).
