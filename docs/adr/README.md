...
| ADR-23 | **One Helm chart** (`helm/ai-platform/`) renders `polaris-dev`/`polaris-staging`/`polaris-prod` from `values.yaml` + one `values-<env>.yaml` each; Phase 4's raw manifests (`deploy/k8s/ai-service/`) are kept as an unmodified reference, not deleted or templated in place | Proves "one chart, three environments" without risking the already-verified Phase 4 YAML; a reference copy is worth more for an interview than a deleted one | Phase 8 (GitOps/Argo CD takes over `helm upgrade --install`); Phase 15 (chart signing/provenance) |
