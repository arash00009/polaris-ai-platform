...
Status: **written; the Docker/Kubernetes-free half runs for real in the sandbox, the rest does not run anywhere yet.** `make lint` and `make test` (127 static checks) pass. `gitleaks v8.30.0` and `actionlint v1.7.12` were downloaded from GitHub release assets and checksum-verified for real; `pip-audit 2.10.1` and `yamllint 1.38.0` were installed from PyPI for real. `make ci-secrets-scan`, `make ci-deps-audit` and `make ci-workflow-lint` all ran against the real repository content and passed — genuinely, not simulated (see `docs/adr/README.md` ADR-24 for the exact results, including a real bug actionlint's embedded shellcheck caught and that was fixed before this was ever pushed). What did **not** run anywhere yet: `docker info` fails in the sandbox (no reachable daemon), so the image build/check/scan/SBOM steps, the ephemeral k3d-in-runner cluster, and the GHCR publish step are shellchecked and `bash -n`-clean but otherwise unverified — their first real run is the first time this workflow actually executes on GitHub Actions after a push, not the target machine (a new category beyond this project's usual sandbox/target-machine split).
...
| 3. Failure | A failing `lint`/`test`/`app-check` stops the pipeline before Docker is even touched (`build-scan-deploy` `needs:` all four fast jobs). A failing `image-scan` (fixable HIGH/CRITICAL) or a failing `helm-smoke-dev` stops it before `publish` ever runs, since `publish` needs `build-scan-deploy` to succeed. Not yet observed for real: this workflow has not executed on GitHub Actions yet, so no failure mode below "written and shellchecked" has actually been triggered by a real run. |
...
| 3. Failure | Not yet observed for real (see the Phase 6 status line above) — this step has never run against a live Docker daemon or a live GitHub Actions runner. By design: a failing `image-scan` (fixable HIGH/CRITICAL) or a failing `helm-smoke-dev` stops the job before `publish` is even considered, and `publish` only runs on a push to `main`, never a pull request, so a fork cannot make an untrusted change reach GHCR. |
