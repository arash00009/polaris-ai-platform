...
| 7 | Continuous verification | Written (not yet run for real) — `scripts/verify/post-deploy.sh` (`make verify-dev/-staging/-prod`): rollout status, health, readyz, a real non-empty AI answer and structured-log presence, all always checked and reported together (not fail-fast); pod resource metrics via k3s's bundled metrics-server are checked but only ever advisory. 139 static checks pass. Its first real run, including a deliberate `POLARIS_MOCK_READY=false` failure-injection test, is on the target machine — same status Phase 4/5's cluster scripts had before their first real run. Not done: a Kubernetes-Job production equivalent (documented, not built — ADR-25); real threshold-based analysis (needs Phase 9/10's Prometheus) |
