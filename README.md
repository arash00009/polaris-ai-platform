...
| 9 | Observability platform: kube-prometheus-stack (Prometheus + Grafana), Loki, Tempo, OTel Collector, ai-service instrumented for metrics/traces/logs | In progress, partially verified on the target machine (2026-09-24). `ai_service` gained always-on Prometheus metrics (`/metrics`, a private `CollectorRegistry` per app instance) and opt-in OTLP traces/logs (`POLARIS_OTEL_ENABLED`, default `false`), instrumented via a real OpenTelemetry SDK. Three application-layer bugs were found and fixed in the sandbox by actually running the code (a self-referential logging loop, an unbounded provider-shutdown now bounded to ~2.9s, a metrics-registry collision across tests); a fourth — a hardcoded version string in a test that broke on this phase's own version bump — was found for real on the target machine and fixed by reading the expected version from `pyproject.toml` instead. `kube-prometheus-stack` itself installed cleanly on the target machine (`STATUS: deployed`, all four pods `Running`); a real Loki chart-usage bug (`deploymentMode: SingleBinary` does not zero the chart's other, still-defaulting-to-3 replica targets, contrary to what its own "deprecated alias" documentation suggests) was found from real `helm install` output and fixed. Loki, Tempo, the OTel Collector and `ai-platform-dev`'s own rollout with `serviceMonitor`/OTLP enabled are not yet confirmed — the target-machine run ended with `make obs-uninstall`, so the next attempt starts clean. The Helm chart and platform manifests default `serviceMonitor.enabled`/`POLARIS_OTEL_ENABLED` to `false` everywhere except `values-dev.yaml`, deliberately, to avoid a ServiceMonitor CRD-ordering failure (ADR-27) — `dev` is the only environment wired up to the observability stack in this delivery. The Grafana Loki/Tempo Helm charts moved to a new upstream repository mid-2026 (confirmed with dated sources); rather than pin a version already shown to disagree between fetches minutes apart, `LOKI_CHART_VERSION`/`TEMPO_CHART_VERSION` are deliberately left unpinned, with the install script printing the real resolved version for a follow-up pin. See `docs/observability.md`, ADR-27, `docs/troubleshooting.md` and `docs/component-qa.md`. Not done: Loki/Tempo/OTel Collector confirmed installed; `ai-platform-dev` confirmed rolled out with observability enabled; Grafana dashboard confirmed showing real data; the resource-request/limit numbers are still Phase 3's guess; OTel trace/log attribute curation and cross-service correlation (Phase 10, explicitly out of scope here) |
