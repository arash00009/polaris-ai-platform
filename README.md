...
| 8 | GitOps with Argo CD (separate configuration repository) | Done (verified 2026-09-23) — `scripts/bootstrap/argocd.sh` installs the pinned, checksum-verified Argo CD `v3.5.2`; a genuinely separate [`polaris-gitops`](https://github.com/arash00009/polaris-gitops) repository holds an app-of-apps root plus one multi-source Helm `Application` per environment. Run for real end to end on the target machine: `make argocd-install` succeeded (all 6 Deployments + 1 StatefulSet `Ready`), `ai-platform-dev`/`-staging` reached `Synced`/`Healthy` automatically with `make verify-dev`/`make verify-staging` both `PASS`, `ai-platform-prod` was promoted through an explicit manual sync with `make verify-prod` `PASS` (`operationState.phase: Succeeded`) — the working stand-in for the roadmap's "manual approval" step, with the honest caveat (ADR-26) that nothing enforces the sync was actually reviewed — and the self-heal demonstration was observed live: a manual `kubectl scale --replicas=1` on `dev` was reverted by Argo CD unprompted, no `kubectl apply`/`helm upgrade` run in between. 169 static checks pass. Four real bugs were found and fixed during this phase (a `kubectl` client-side-apply CRD annotation-size limit, a `bump-image-tag.sh` idempotency bug, a rollout timeout too low for this network's first image pull, and a static-check gap) — see `docs/troubleshooting.md` and ADR-26. Not done: image-tag promotion into CI (deliberately left manual this phase, ADR-26) |
| 9 | Observability platform: kube-prometheus-stack (Prometheus + Grafana), Loki, Tempo, OTel Collector, ai-service instrumented for metrics/traces/logs | In progress — metrics and logs confirmed with real data on the target machine (2026-09-25); traces not yet confirmed, see below. `ai_service` gained always-on Prometheus metrics (`/metrics`, a private `CollectorRegistry` per app instance) and opt-in OTLP traces/logs (`POLARIS_OTEL_ENABLED`, default `false`), instrumented via a real OpenTelemetry SDK. Three application-layer bugs were found and fixed in the sandbox by actually running the code (a self-referential logging loop, an unbounded provider-shutdown now bounded to ~2.9s, a metrics-registry collision across tests). Five more real bugs were found on the target machine across three installation runs (2026-09-24), each fixed and confirmed: a hardcoded version string in a test, a Loki chart-usage bug (`deploymentMode: SingleBinary` does not zero the chart's other, still-defaulting-to-3 replica targets), `kube-prometheus-stack`'s rollout timeout needing the same 600s→900s bump Argo CD needed in Phase 8, the OTel Collector chart's `image.repository` now being required explicitly, and Grafana's memory limit (256Mi→512Mi) being too low for its newer unified-storage subsystem, causing repeated OOMKilled restarts. All four Helm releases (`kube-prometheus-stack`, Loki, Tempo, OTel Collector) install cleanly to `STATUS: deployed` together. `ai-platform-dev`'s rollout with `serviceMonitor`/OTLP enabled is confirmed live via GitOps (2026-09-25): new image tag running, pods healthy. The Grafana dashboard was opened and checked against real data (2026-09-25): request rate, latency percentiles, pod restarts/CPU/memory and recent application logs (`service_name="ai-service"`, populated only once real `/v1/chat` traffic was sent — health probes deliberately don't log) all show real data; the error-rate panel correctly shows no data, since no 4xx/5xx traffic has occurred yet. Traces are the one signal not yet confirmed reaching Tempo/Grafana — the dashboard has no trace panel, and `ai-service`'s logs show a handful of unexplained OTLP span-export timeouts against the Collector, not yet root-caused. The Helm chart and platform manifests default `serviceMonitor.enabled`/`POLARIS_OTEL_ENABLED` to `false` everywhere except `values-dev.yaml`, deliberately (ADR-27) — `dev` is the only environment wired up to the observability stack in this delivery. `LOKI_CHART_VERSION`/`TEMPO_CHART_VERSION` are now pinned (Loki `18.13.5`, Tempo `3.0.0`) after the Grafana Loki/Tempo Helm charts' move to a new upstream repository mid-2026. See `docs/observability.md`, ADR-27, `docs/troubleshooting.md` and `docs/component-qa.md`. Not done: traces confirmed reaching Tempo/Grafana; OTel Collector's deprecated `otlphttp` exporter names updated to `otlp_http`; the resource-request/limit numbers for the other three observability releases are still Phase 3-era guesses, untested under load; OTel trace/log attribute curation and cross-service correlation (Phase 10, explicitly out of scope here) |
| 10 | OpenTelemetry: trace/log attribute curation, log↔trace correlation, cross-service trace propagation | Planned |
