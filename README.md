...
| 9 | Observability platform: kube-prometheus-stack (Prometheus + Grafana), Loki, Tempo, OTel Collector, ai-service instrumented for metrics/traces/logs | Written and shellchecked in the sandbox (196 static checks pass; 141 `ai_service` unit tests at 99.27 % coverage), **not yet installed on the target machine** — this row will be updated once `make obs-install` and the ai-platform Helm upgrade have actually run there. `ai_service` gained always-on Prometheus metrics (`/metrics`, a private `CollectorRegistry` per app instance — not the process-global one, so tests don't collide) and opt-in OTLP traces/logs (`POLARIS_OTEL_ENABLED`, default `false`), instrumented via a real OpenTelemetry SDK, not a stub. Three real application-layer bugs were found and fixed by actually running the code, not by reasoning about it: a self-referential logging loop when the OTel log handler was attached to the root logger instead of `ai_service`'s own; an unbounded provider-shutdown that measured ~8s against an unreachable collector, now bounded to ~2.9s by an explicit `otel_exporter_timeout_s`; and a metrics-registry collision across tests, fixed the same way as the logging fix — scope it, don't share it globally. The Helm chart and platform manifests default `serviceMonitor.enabled`/`POLARIS_OTEL_ENABLED` to `false` everywhere except `values-dev.yaml`, deliberately, to avoid a ServiceMonitor CRD-ordering failure (ADR-27) — `dev` is the only environment wired up to the observability stack in this delivery. The Grafana Loki/Tempo Helm charts moved to a new upstream repository mid-2026 (confirmed with dated sources); rather than pin a version already shown to disagree between fetches minutes apart, `LOKI_CHART_VERSION`/`TEMPO_CHART_VERSION` are deliberately left unpinned, with the install script printing the real resolved version for a follow-up pin. See `docs/observability.md`, ADR-27, `docs/troubleshooting.md` and `docs/component-qa.md`. Not done: anything target-machine-verified; the resource-request/limit numbers are still Phase 3's guess (Phase 9 makes the data to fix that *possible*, not fixed); OTel trace/log attribute curation and cross-service correlation (Phase 10, explicitly out of scope here) |
