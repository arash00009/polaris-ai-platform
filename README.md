...
| 7 | Continuous verification | In progress (target machine, 2026-09-22) — `scripts/verify/post-deploy.sh` (`make verify-dev/-staging/-prod`): rollout status, health, readyz, a real non-empty AI answer and structured-log presence, all always checked and reported together (not fail-fast); pod resource metrics via k3s's bundled metrics-server are checked but only ever advisory. 140 static checks pass. The healthy path is verified for real against `dev`/`staging`/`prod`, all six checks passing with genuine metrics. The deliberate `POLARIS_MOCK_READY=false` failure-injection test found a real gap instead of exercising the one it was written for: Phase 5's chart had no `checksum/config` annotation, so a ConfigMap-only value change never triggers a rollout — the injected change never reached a running pod (confirmed via identical pod names/ReplicaSet hash before and after), so nothing was actually unhealthy for the script to catch. The fix (a `checksum/config` annotation, `helm/ai-platform/templates/deployment.yaml`) is written and its hashing mechanism verified in the sandbox; re-confirming a genuine caught failure on the target machine is the one remaining step — see `docs/troubleshooting.md` and ADR-25. Not done: a Kubernetes-Job production equivalent (documented, not built — ADR-25); real threshold-based analysis (needs Phase 9/10's Prometheus) |
