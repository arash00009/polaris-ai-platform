...
| 9 | Observability platform: kube-prometheus-stack (Prometheus + Grafana), Loki, Tempo, OTel Collector, ai-service instrumented for metrics/traces/logs | In progress, partially verified on the target machine across two real runs (2026-09-24). `ai_service` gained always-on Prometheus metrics (`/metrics`, a private `CollectorRegistry` per app instance) and opt-in OTLP traces/logs (`POLARIS_OTEL_ENABLED`, default `false`), instrumented via a real OpenTelemetry SDK. Three application-layer bugs were found and fixed in the sandbox by actually running the code (a self-referential logging loop, an unbounded provider-shutdown now bounded to ~2.9s, a metrics-registry collision across tests). Four more real bugs were found on the target machine, each fixed and each confirmed or still-to-confirm as noted: (1) a hardcoded version string in a test that broke on this phase's own version bump — fixed and confirmed (`make test` passes clean); (2) a Loki chart-usage bug (`deploymentMode: SingleBinary` does not zero the chart's other, still-defaulting-to-3 replica targets, contrary to what its own "deprecated alias" documentation suggests) — fixed and confirmed (`helm install` reaches `STATUS: deployed` for Loki); (3) `kube-prometheus-stack`'s rollout timeout needing the same 600s→900s bump Argo CD needed in Phase 8 — fixed and confirmed; (4) the OTel Collector chart's `image.repository` now being required explicitly, a breaking chart-default change — fixed, not yet reconfirmed. As of the second run, `kube-prometheus-stack`, Loki and Tempo all install cleanly; the OTel Collector and `ai-platform-dev`'s own rollout with `serviceMonitor`/OTLP enabled are not yet confirmed. The Helm chart and platform manifests default `serviceMonitor.enabled`/`POLARIS_OTEL_ENABLED` to `false` everywhere except `values-dev.yaml`, deliberately, to avoid a ServiceMonitor CRD-ordering failure (ADR-27) — `dev` is the only environment wired up to the observability stack in this delivery. The Grafana Loki/Tempo Helm charts moved to a new upstream repository mid-2026 (confirmed with dated sources); rather than pin a version already shown to disagree between fetches minutes apart, `LOKI_CHART_VERSION`/`TEMPO_CHART_VERSION` are deliberately left unpinned, with the install script printing the real resolved version for a follow-up pin (now known from the second run: Loki `18.13.5`, Tempo `3.0.0`). See `docs/observability.md`, ADR-27, `docs/troubleshooting.md` and `docs/component-qa.md`. Not done: OTel Collector confirmed installed; `ai-platform-dev` confirmed rolled out with observability enabled; Grafana dashboard confirmed showing real data; the resource-request/limit numbers are still Phase 3's guess; OTel trace/log attribute curation and cross-service correlation (Phase 10, explicitly out of scope here) |
